...
httpInstance.interceptors.request.use((config) => {
  ...
  //     configs.baseURL='https://apijp.tokenew.io/'
  //   }
  // }
  if (localStorage.getItem('wallet_token')) {
    ...
    if (configs.method == "post") {
      configs.data.token = localStorage.getItem('wallet_token');
      // if (configs.url.indexOf('/cloud/backmgr/financial/coin/addCoinDetail')>-1 || configs.url.indexOf('/cloud/backmgr/financial/coin/updateCoinDetail')>-1) {
      //   configs.body.context =  JSON.parse(JSON.stringify(configs.data.context));
      //   configs.data.context = '';
      //   console.log(configs, 999);
      // }
    } else {
      configs.params.token = localStorage.getItem('wallet_token');
    }
