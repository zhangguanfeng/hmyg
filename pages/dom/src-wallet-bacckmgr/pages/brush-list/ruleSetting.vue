...
<template>
  <div class='ruleSetting'>
    ...
    <sac-table :data="registList">
      <el-table-column prop="coinName" label="返佣币种">
      </el-table-column>
      <el-table-column prop="direct" label="直推返佣">
        <template slot-scope="scope">
          {{scope.row.direct*100 || 0}}%
        </template>
      </el-table-column>
      <el-table-column prop="lower" label="初级代理提成">
        <template slot-scope="scope">
          {{scope.row.lower*100 || 0}}%
        </template>
      </el-table-column>
      <el-table-column prop="middle" label="中级代理提成">
        <template slot-scope="scope">
          {{scope.row.middle*100 || 0}}%
        </template>
      </el-table-column>
      <el-table-column prop="senior" label="高级代理提成">
        <template slot-scope="scope">
          {{scope.row.senior*100 || 0}}%
        </template>
      </el-table-column>
      <el-table-column prop="service" label="服务中心提成">
        <template slot-scope="scope">
          {{scope.row.service*100 || 0}}%
        </template>
      </el-table-column>
      <el-table-column prop="peers" label="平级补助">
        <template slot-scope="scope">
          {{scope.row.peers*100 || 0}}%
        </template>
      </el-table-column>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible" class="register_dialog">
      <el-form :model="ruleForm" ref="ruleForm" :rules="rules" label-width="135px">
        ...
        <el-form-item label="直推返佣提成：" prop="direct" class="from_box_item">
          <el-input type="number" size="small" v-model="ruleForm.direct" placeholder="">
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          ...
          direct: [
            {required: true, message: '请输入直推返佣提成', trigger: 'blur'},
          ],
          lower: [
    ...
    methods: {
      ...
      modification(itemData) {
        ...
        this.ruleForm.direct = this.ruleForm.direct * 100;
      ...
      registDetermine() {
        this.$refs.ruleForm.validate((valid) => {
          if (valid) {
            const {coinId, id, lower, direct,  middle, senior, service, peers} = this.ruleForm;
            if (this.ruleForm.id) {
