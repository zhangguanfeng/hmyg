 ...
 * @Author: Diskfan 
 * @Content: 放行 dialog
 * @Date: 2019-06-13 11:43:28 
 * @Last Modified by: Diskfan
 * @Last Modified time: 2019-06-18 10:26:57
 */
<template>
    <div>
...
export default {
    ...
    methods: {
        ...
        confirm () {
            ...
            if (this.type === 'auto') {
                params.sysUserId = '123456'
                params.recdId = this.item.recdId
                params.recdStatus = 'SUCCESS'
                params.sysRemark = form.remark
                this.$http
                    .post(`${this.$cinfog.fundAddr}/backmgr/auditOrder`, params)
                    .then(data => {
                        if (data.ret === 0) {
                            this.$notify.success({
                                title: '提示',
                ...
                this.$http
                    .get(`${this.$cinfog.fundAddr}/backmgr/manualAudit`, params)
                    .then(({ data }) => {
                        if (data.ret === 0) {
                            this.$notify.success({
                                title: '提示',
                ...
                const params = {
                    ...
                    recdStatus: 'AUDIT_FAIL',
                    sysRemark: form.reason,
                }
                this.$http
                    .post(`${this.$cinfog.fundAddr}/backmgr/auditOrder`, params)
                    .then(({ code }) => {
