...
export default {
    ...
    methods: {
		...
		downloadExcel() {
            ...
            window.open(this.$config.fundAddr + '/backmgr/open/opsExportBlockRecdList?' + qs.stringify(params))
        ...
        allCoins () {
            this.$http.get(`${this.$config.fundAddr}/backmgr/coin/getAllCoinInfo`)
                .then(({ code, result }) => {
                    if (code === 200) {
                        let USDT = false
