      ...
      <div class="layer" @click="layerClick"></div>
        ...
        <div class="tncode_div" id="tncode_div">
          ...
          <div class="slide">
            <div
                class="slide_block"
                @touchstart="_block_start_move"
                @touchmove="_block_on_move"
                @touchend="_block_on_end"
            ></div>
    ...
    methods: {
      layerClick() {
        ...
        refresh() {
            ...
            var img_url =
                SERVER_PATH + "/Home/tncode?t=" + this.tncode.ramdon_t;
            if (!isSupportWebp) {
                //浏览器不支持webp
                img_url += "&nowebp=1";
            }
        ...
        _block_on_move(e) {
            ...
            this.tncode._mark_offset =
                (offset / max_off) * (this.tncode._img_w - this.tncode._mark_w);
            //this._draw_bg();
            this._draw_mark();
        },
        _block_on_end(e) {
        ...
        _draw_mark() {
            ...
            var imageData = ctx_mark.getImageData(
                0,
                0,
                this.tncode._img_w,
                this.tncode._img_h
            ...
            //alert(data.length/4);
            var x = this.tncode._img_h,
                y = this.tncode._img_w;
            for (var j = 0; j < x; j++) {
                var ii = 1,
                    k1 = -1;
