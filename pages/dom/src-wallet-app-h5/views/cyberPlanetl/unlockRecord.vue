<template>
  <div class='extractRecord'>
    ...
    <div class="title">
      <span class="left">{{title[0]}}</span> <span class="sum">{{title[1]}}</span> <span class="account">{{title[2]}}</span> <span class="account">{{title[3]}}</span>
    </div>
    <scroller lock-x class="scroller" @on-scroll-bottom="onScrollBottom" ref="scrollerBottom"
      ...
      <div>
        <div v-for="(item, index) in list" class="list" :key="index">
          ...
          <span class="sum" style="color: #082C71;   font-weight:600;">{{item.currency}}</span>
          <span class="account" style="color: #999;">{{item.amount}}</span>
          <span class="account" style="color: #999;">{{item.status}}</span>
        </div>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$http2.post('invest/financeApi/getExWithdrawList', {
        page: this.pageNum,
